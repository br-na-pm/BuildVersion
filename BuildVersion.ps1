...
if($LocalProgramFound) {
    if([System.IO.File]::Exists($LocalVariableFile)) {
        ...
        if($FileMatch.Success) {
            ...
            Write-Host "BuildVersion: Local variable file verified"
        }
        else {
            Write-Host "BuildVersion: The content of local variable file $LocalVariableFile could not be verified"
        }
    }
    # Generate blank local variable file to avoid build error
    else {
        Write-Host "BuildVersion: No local variable file found in $ProgramPath, generating blank file"
        $FileContent = @"
...
if((-not $GlobalOption) -and (-not $LocalProgramFound)) {
