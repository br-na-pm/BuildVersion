...
if($ProgramFound) {
    $LocalFile = $ProgramPath + "Variables.var"
    $RelativeLocalFile = $LocalFile.Replace($LogicalPath, ".\Logical\")
    if([System.IO.File]::Exists($LocalFile)) {
        ...
        if($FileMatch.Success) {
            ...
            Set-Content -Path $LocalFile $FileContent
            Write-Host "BuildVersion: Local variable file $RelativeLocalFile verified"
        }
        else {
            Write-Warning "BuildVersion: Local variable file $RelativeLocalFile unverified"
        }
    }
...
"@
    ...
    if($Match.Success) {
        Write-Host "BuildVersion: $RelativeLocalFile updated with build version information"
    }
    else {
        $Content = @"
(*This file was automatically generated by $ScriptName on $BuildDate.*)
...
END_VAR
"@
        Set-Content -Path $LocalFile $Content
        Write-Host "BuildVersion: $RelativeLocalFile overwritten with build version information"
    }
}
