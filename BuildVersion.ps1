...
$SearchDirectory = Get-ChildItem -Path $LogicalPath -Filter $ProgramName -Recurse -Directory -Name
$ProgramFound = $False
# Loop through zero or more directories named $ProgramName
foreach($SearchDirectoryItem in $SearchDirectory) {
    $ProgramPath = $LogicalPath + $SearchDirectoryItem + "\"
    # Search for any file in this directory with extension .prg
    $SearchFileResults = Get-ChildItem -Path $ProgramPath -Filter "*.prg" -Name
    # If there is at least one *.prg file assume Automation Studio program
    if($SearchFileResults.Count -ne 0) {
        $ProgramFound = $True
...
$SearchFileResults = Get-ChildItem -Path $LogicalPath -Filter $GlobalDeclarationName -Recurse -File -Name 
$GlobalVariableFileFound = $False
foreach($SearchFileItem in $SearchFileResults) {
    $GlobalVariableFile = $LogicalPath + $SearchFileItem
    $GlobalVariableFileFound = $True
    $RelativeGlobalVariableFile = $GlobalVariableFile.Replace($LogicalPath, ".\Logical\")
    Write-Host "BuildVersion: Located $GlobalDeclarationName at $RelativeGlobalVariableFile"
    # Only take the first Global.var found
    break
}
if(-not $GlobalVariableFileFound) {
...
if($ProgramFound) {
    ...
    $RelativeLocalVariableFile = $LocalVariableFile.Replace($LogicalPath, ".\Logical\")
    if([System.IO.File]::Exists($LocalVariableFile)) {
        $FileContent = Get-Content $LocalVariableFile
        $FileMatch = [regex]::Match($FileContent, "\(\*This file was automatically generated by ([\w\.]+) on ([\d-:]+)\.\*\)")
        if($FileMatch.Success) {
            $FileContent = $FileContent.replace($FileMatch.Groups[1].Value, $ScriptName)
            $FileContent = $FileContent.replace($FileMatch.Groups[2].Value, $BuildDate)
            Set-Content -Path $LocalVariableFile $FileContent
            Write-Host "BuildVersion: Local variable file $RelativeLocalVariableFile verified"
        }
        else {
            Write-Warning "BuildVersion: Local variable file $RelativeLocalVariableFile unverified"
        }
    }
    # Generate blank local variable file to avoid build error
    else {
        # This will occur when first cloning a project so it remains informational
        Write-Host "BuildVersion: No local variable file found in $RelativeProgramPath. Generating blank file."
        $FileContent = @"
...
if($ProgramFound) {
    ...
    $Regex = @"
(?x)
BuildVersion \s* : \s* BuildVersionType \s*
(?: := \s* \(
(?: \s* 0+ )?
(?: \s* Script \s* := \s* \( ( [^\(\)]+ ) \) )?
\s* ,? \s*
...
\) \s* )? ;
"@
    $FileMatch = [regex]::Match($FileContent, $Regex)
    $FileContent = @"
(*This file was automatically generated by $ScriptName on $BuildDate.*)
(*Do not modify the contents of this file.*)
VAR
